 ...
 *    18 July 2019:  irrational roots: irroots3-2.cpp
 *                     = "irroots2"
 *
 *       This, a second version of irroots3.cpp
 *       where user chooses significant figures
 *      ***   will be compiled as irroots2  ***
 *
 *** * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
//    using endpoints a to b subintervals dx.
  ...
  // For second version with "query least number of significant
  // figures required."
  FIGURES = setSigFig(getSigFig());
...
std::cin >> choice;
 ...
 if ( (choice == 'y') || (choice == 'Y') )  {
     ...
     if (RSD+1 < FIGURES) std::cout << "only ";
     std::cout << "good to " << RSD+1
               << " significant figures.\n\n";  // RSD = 1, so 2 figures
     if (RSD+1 < FIGURES)
        std::cout << "\nYou must answer NO until the required significant"
                  << " figures are reached.\n\n";
     return 0;
  }
  else
...
/*  REPLACING
  int FIGURES = 7;         // from irroots3.cpp

// Since setting to 1 or 2 gives 4 figures of accuracy,
// if required figures is <= 4, set to 4/2 = 2.
// Since setting to 3 or 4 gives 8 figures of accuracy,
// if required figures is <= 8, set to 8/2 = 4.
// Since setting to 5, 6, 7, 8 gives 16 figures of accuracy,
// if required figures is <= 16, set to 16/2 = 8.
// Since setting to 9..15 gives 32 figures of accuracy,
// if required figures is <= 32, set to 32/2 = 16.
// The order must be reversed and second variable G initialized to 32.
 */
 int setSigFig(int F)  {
   int G = 32;
   if (F <= 16) G = 16;
